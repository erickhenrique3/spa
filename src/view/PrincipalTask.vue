<template>
	<div class="container">
		...
		<ModalCreateSubtask
			...
			:formSubtaskSubmitted="formSubtaskSubmitted"
			@close="ShowModalSub = false"
			@refresh-tasks="getTasks"
		/>

		<ModalUpdateSubtask
			:showUpdateModalSub="showUpdateModalSub"
			:taskToUpdateSub="taskToUpdateSub"
			:formSubtaskSubmitted="formSubtaskSubmitted"
			@close="showUpdateModalSub = false"
			@refresh-tasks="getTasks"
		/>
		...
		<div class="modal-task" v-if="openTaskModal">
			<div class="modal-body-task">
				<div class="modal-header-task">
					...
					<div class="dropdown">
						...
						<div class="dropdown-content">
							...
							<a @click="printTask"
								><i class="bx bx-printer"></i> Imprmir tarefa</a
							>
							<a @click="deleteTask(selectedTask.id)" style="color: red"
								><i class="bx bx-trash"></i>Deletar tarefa</a
							>
						</div>
					</div>
				...
				<div class="box1-modal-task">
					<label class="custom-radio">
						<input
							...
							type="radio"
							:id="'task-status-' + selectedTask.id"
							v-model="selectedTask.status"
							value="completed"
						/>
						<span
							class="checkmark"
							:class="{ checked: selectedTask.status === 'completed' }"
						></span>
					</label>
					...
					<div class="dropdown2">
						...
						<div class="dropdown-content2">
							<a @click="openModalSub">
								<i class="bx bx-plus"></i>Criar subtarefa
							</a>
						</div>
					</div>
					<hr class="hr_sub" />
					...
					<div
						class="fullSubtasks"
						v-if="selectedTask.subtasks && selectedTask.subtasks.length"
					>
						<div
							class="allSubtasks"
						...
						>
							...
							<div
								class="subtasks"
								@mouseenter="showSubtaskIcons(subtask.id)"
							...
							>
								...
								<div class="subtask-icons" v-show="subtask.showIcons">
									<i
										@click.stop="openUpdateSubtask(subtask)"
				...
				<div class="box2-modal-task">
					...
					<span
						:style="{
							backgroundColor: BackgroundColorDate(selectedTask.due_date),
						}"
						><strong
							><i class="bx bx-notepad"></i
							>{{ formatDate(selectedTask.due_date) }}</strong
						></span
					>
		...
		<ModalUpdateTask
			...
			@close="closeUpdateTask"
		/>

		<ModalUpdateDate
			:showUpdateDate="showUpdateDate"
			:dataDueDate="taskToUpdateDate.due_date"
			@close="closeUpdateDate"
			:taskToUpdateDate="taskToUpdateDate"
		...
		<div class="box1">
			<div>
				<button @click="showAllTasks()">
					<i class="bx bx-store-alt" style="color: rgba(32, 32, 32, 0.92)"></i>
					Entrada
				</button>
			...
			<div class="conteudo">
				...
				<div class="card-container">
					<div
						...
						:key="task.id"
						@click="openTaskModalClick(task)"
						@mouseover="task.ShowIcons = true"
						@mouseleave="task.ShowIcons = false"
					>
						<label class="custom-radio" @click.stop="stopModal">
							<input
								@click="stopModal, updateTaskStatus(task)"
								type="radio"
						...
						<div class="icons-task">
							<div class="container-icons" v-show="task.ShowIcons">
								<i
									@click.stop="openUpdateTask(task)"
									class="bx bx-edit-alt"
								></i>
								<i
									@click.stop="
						...
						<span
							class="data"
							:style="{ backgroundColor: BackgroundColorDate(task.due_date) }"
						>
							<i class="bx bx-notepad"></i>{{ formatDate(task.due_date) }}</span
						>
						<hr />
						<div
							v-if="
						...
						<button @click.stop="ShowModal = true">
							<i class="bx bx-plus" style="color: #000"></i> Criar tarefa
						</button>
					</div>
				</div>
			</div>
...
export default {
	...
	methods: {
		...
		showSubtaskIcons(subtaskId) {
			const subtask = this.selectedTask.subtasks.find(
		...
		updateTaskStatus(task) {
			...
			axios
				.patch(`tasks/${task.id}/status`, { status: newStatus })
		...
		updateSubtaskStatus(subtaskId, _currentStatus, newStatus) {
			axios
				.patch(`/tasks/subtasks/${subtaskId}`, { status: newStatus })
				.then(() => {
					this.getTasks();
				});
		},
		...
		deleteSubtask(subtask) {
			axios.delete(`/tasks/subtasks/${subtask.id}`).then(() => {
				this.openTaskModal = false;
				this.getTasks();
			});
		},
